...
<div class="top_bar <?php if(!empty($page_menu_transparent)) { ?>hasbg<?php } ?>">
			...
			<div id="logo_wrapper">
			<?php
				if($tg_menu_layout == 'centeralign2')
			{
		?>
		...
		<?php
			...
			else if($tg_menu_layout == 'centeralign3')
			{
				get_template_part("/templates/template-menu-address");
			}
			else
			{
				//Get Soical Icon
		...
		<div id="logo_right_button">
			...
			<!-- ZD Mod -->
			<div id="lang-nav"><?php echo do_shortcode( '[wpml_language_selector_widget post_id='.$post->ID.']' ); ?></div>

			<?php
					if($tg_menu_layout == 'centeralign3')
				{
			?>
			...
			<?php
				if($tg_menu_layout != 'centeralign2' && $tg_menu_layout != 'centeralign3')
				{
			?>
				 <!-- Begin side menu -->
				 <a href="javascript:;" id="mobile_nav_icon"><span class="ti-menu"></span></a>
			...
			<div class="header_call_wrapper">
					<span class="ti-mobile"></span><a class="tooltip" href="tel:<?php echo esc_html($tg_menu_contact_number); ?>" title="<?php esc_html_e('Call Us', 'hoteller' ); ?>"><?php echo esc_html($tg_menu_contact_number); ?></a>
			</div>
			<?php
					}
				}
			?>
			<?php get_template_part( 'templates/booking-button' ); ?>
			...
			<div id="logo_transparent" class="logo_container">
				<div class="logo_align">
					...
					<?php
						...
						{
					?>
							<img src="<?php echo esc_url($tg_retina_transparent_logo); ?>" alt="<?php esc_attr(get_bloginfo('name')); ?>" class="custom_logo_no_info"/>
							<?php 
								}
						else
						{
